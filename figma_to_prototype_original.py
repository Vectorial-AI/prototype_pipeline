...
---------------------
Converts any Figma file JSON + images into a verified, self-contained HTML prototype.
...
def _collect_interactions(node, frame_ids, hotspots, toggles, timeouts, parent_frame_id=None, ox=0, oy=0, depth=0):
    ...
    # Determine if this node is a top-level frame
...
window.addEventListener('load',()=>{{
  ...
  else console.log('✓ {len(frames)} screens | {total_nav} nav hotspots | {total_tog} toggles | {len(timeouts)} auto-advances');
}});
go(ENTRY);hist=[ENTRY];hnames=[FRAMES[ENTRY]?.name||''];
</script>
</body>
...
def main():
    ...
    ap.add_argument('--figma-file',  required=True)
    ap.add_argument('--figma-token', default='')
    ...
    print(f"\n1. Loading {args.figma_file}...")
    with open(args.figma_file) as f:
    ...
    print(f"\n4. Entry frame: {frames[entry_id]['name']} [{entry_id}]")

    img_dir = os.path.join(args.output_dir, 'images')
    if args.images_dir:
        print(f"\n5. Using local images from {args.images_dir}...")
        image_map = use_local_images(frames.keys(), args.images_dir, img_dir)
    elif args.figma_token and args.file_key:
        print(f"\n5. Downloading {len(frames)} images from Figma API...")
        image_map = download_images(frames.keys(), args.file_key, args.figma_token, img_dir)
    else:
        print("\n5. No image source — pass --figma-token + --file-key OR --images-dir")
        image_map = {}
    ...
    if issues:
        ...
        print(f"   ✓ {len(frames)} screens | {total_nav} nav | {total_tog} toggles | {len(timeouts)} timers — all verified")
